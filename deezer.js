...
class Deezer {
		...
		return new Promise((resolve, reject) => {
			...
			api.get('deezer', '/user/me/flow', settings.deezer.access_token, { output: 'json' }, (err, result) => {
				...
				for (let i of result.data) 
					data.deezer[0].tracks.push(convertTrack(i));
	...
	*/
	...
	static viewArtist (track) {
		...
		request({ url:  api_url+'/artist/'+track.artist.id+'/top', method: "GET", json: true}, (err, result, body) => {
			if (err) return console.error(err);

			let tracks = [];

			for (let tr of body.data)
				tracks.push(convertTrack(tr));

			specialView('deezer', tracks, 'artist', track.artist.name, body.data[0].contributors[0].picture);
		});
	}
	...
	static viewAlbum (track) {
		...
		request({ url:  api_url+'/album/'+track.album.id, method: "GET", json: true}, (err, result, body) => {
		  if (err) return console.error(err);

		  let tracks = [];

		  for (let tr of body.tracks.data){
				i.album = { title: track.album.name, id: track.album.id };
				tracks.push(convertTrack(tr));
		  }

		  specialView('deezer', tracks, 'album', track.album.name, body.cover_medium);
		});
	}
	...
	*/
	static searchTracks (query, callback) {
		...
		request({ url:  api_url+'/search&q='+encodeURI(query), method: "GET", json: true}, (err, result, body) => {
